...
\newcommand{\parselify}[1]{%
  \StrSubstitute{#1}{с}{сс}[\parselstringD]%
  \StrSubstitute{\parselstringD}{С}{Сc}[\parselstringC]%
  \StrSubstitute{\parselstringC}{ш}{шш}[\parselstringB]%
  \StrSubstitute{\parselstringB}{Ш}{Шш}[\parselstringA]%
  % ...
  \StrSubstitute{\parselstringA}{}{}[\parselstringFINAL]
}
...
{\large\hpFontParsel\parselstringFINAL}}
...
\newcommand{\McGonagallWhiteBoard}[1]{%
  \begin{center}
    \newsavebox{\hpbox}%
    \fontspec[ExternalLocation,Color=AA0000]{BadScript-Regular}
...
% Letters / handwritten fonts
\newfontface\hpFontCursive[ExternalLocation]{GreatVibes-Regular.ttf}
% Fast handwriting font
\newfontface\hpFontFasthand[ExternalLocation]{Caveat-Regular.ttf}
% Font used in notes from Game of Life
\newfontface\hpFontGame[ExternalLocation]{Pacifico-Regular.ttf}




\newenvironment{writtenNote}{%
\begin{flushleft}  % Yep, nobody can justify ideally handwritten text, so lets not even pretend.
\hpFontFasthand\large%
...
\itshape% use cursive fonts already, but ok
}
{%
\par
\end{adjustwidth}%
\end{flushleft}
\vskip 1\baselineskip plus 1\baselineskip minus 0.1\baselineskip%
}
\newenvironment{writtenNoteCursive}{\begin{writtenNote}\Large\hpFontCursive}{\end{writtenNote}}
\newenvironment{writtenNoteGame}{\begin{writtenNote}\hpFontGame}{\end{writtenNote}}

% \letterAddress
\newcommand{\letterAddress}[1]{%%\fontspec[ExternalLocation]{Graphe_Alpha_alt.ttf}%
\pagebreak[1]#1\nopagebreak[4]\par%
}
...
\newcommand{\partchapter}[3][\relax]{%
	\chapter[\texorpdfstring{#2, \abbrev{частка #3}}{#2, частка #3}]%
		[#2 #3]{#2, Частка~#3#1}}
\newcommand{\namedpartchapter}[5][\relax]{%
	\chapter[%
			\texorpdfstring{%
				...
				#3, частка #4: #5}][%
			\mbox{#2 #4:} \mbox{#5}]{%
			#2, Частка~#4:\protect\linebreak[1] #5#1}%
}
...
% Large, normal, tiny
\def\Stars{{{\large\Star}\kern-.6ex\lower1.3ex\hbox{\Star}\kern-.1ex\raise.2ex\hbox{\tiny\Star}\spacefactor1000}}
